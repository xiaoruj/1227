  ...
  <el-form label-width="100px" v-show="visible">
    <el-form-item label="SPU名称">
      <el-input
        type="text"
        placeholder="SPU名称"
        v-model="spuInfo.spName"
      ></el-input>
    </el-form-item>
    <el-form-item label="品牌">
      <el-select placeholder="选择品牌" value="spuInfo.tmId">
        <el-option
          :label="tm.tmName"
          :value="tm.id"
...
export default {
  ...
  computed: {
    unUsedSaleAttrList() {
      return this.saleAttrList.filter(attr =>
        this.spuInfo.spuSaleAttrList.evert(
  ...
  methods: {
    ...
    handleInputConfirm(spuSaleAttr) {
      const { saleAttrName, baseSaleAttrId } = spuSaleAttr;
      if (!saleAttrName) {
